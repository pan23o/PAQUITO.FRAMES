(function(){
  ...
  async function saveProfile(){
    ...
    try{
      ...
      if(error)throw error;
      if(data){
        $('profileBrand').value=data.brand||'';$('profileInstagram').value=data.instagram||'';$('profileEmail').value=data.email||'';$('profileLocation').value=data.location||'';$('profileBio').value=data.bio||'';renderPhoto(data.about_photo_url||aboutPhotoUrl);
      }
  ...
  async function removeAboutPhoto(){
    ...
    try{
      ...
      renderPhoto('');
      photoStatus('Fotografía eliminada.',true);
